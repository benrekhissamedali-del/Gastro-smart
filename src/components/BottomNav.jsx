const NAV = [
  ...
  { id: 'add',       icon: PlusIcon,      label: 'جديد',   isAction: true },
  { id: 'analytics', icon: ChartIcon,     label: 'تحليلات' },
    ...
    >
      {NAV.map(item => {
        ...
        return (
          <button
            key={item.id}
            onClick={() => isAction ? onAdd() : onNavigate(item.id)}
            style={{
              flex: 1,
              display: 'flex',
...
function ChartIcon({ size, color }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <path d="M3 3V19C3 19.5523 3.44772 20 4 20H21" stroke={color} strokeWidth="1.8" strokeLinecap="round"/>
      <path d="M7 14L11 9L14 12L18 6" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}
